...
def index():
    ...
    if request.method == 'POST':
        ...
        result = cur.execute("select student_id, student_name, guardian_name,guardian_number from students where class=%s and section=%s",
                             (user_input_class, user_input_section))
        result1 = cur1.execute("SELECT status from student_attendance_map where attendance_date=%s", [
                               user_input_attendance])
...
def update():
    ...
    for i in data['html_data']:
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO student_attendance_map (attendance_date, status, student_id, set_at) VALUES (%s, %s, %s,1) ",
                    (user_input_attendance, i[0], i[1]))
...
def update_attd():
    ...
    for i in data['html_data']:
        ...
        cur.execute(
            "UPDATE student_attendance_map set status=%s where attendance_date=date(%s) and student_id=%s", (i[0], s1, i[1]))
        mysql.connection.commit()
        cur.close()
